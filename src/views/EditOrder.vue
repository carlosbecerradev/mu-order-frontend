<template>
  <div class="">
    <h1>Editar pedido</h1>
    <form @submit.prevent="editOrder()" class="">
      <!-- Seleccionar item -->
  ...
  methods: {
    ...
    async getOrderById(id) {
      try {
        ...
        if(response.status == 200){
          const responseBody = await response.json();
          this.fillEditForm(responseBody);
        }
      } catch (error) {
        console.error(error)
      }
    },
    fillEditForm(orderResponse) {
      this.filterByItem.itemNameParameter = orderResponse.item.name;
      this.getItemOptionsAndTypes(orderResponse.item.id);
      console.log(this.orderRequest)

      this.orderRequest.id = orderResponse.id;
      this.orderRequest.itemOption = orderResponse.itemOption;
      this.orderRequest.itemType = orderResponse.itemType;
      this.orderRequest.itemLevel = orderResponse.itemLevel;
      this.orderRequest.observation = orderResponse.observation;
      this.orderRequest.itemId = orderResponse.item.id;
    },
    async editOrder() {
      try {
